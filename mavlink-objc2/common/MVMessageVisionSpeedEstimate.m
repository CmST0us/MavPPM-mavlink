...
- (id)initWithSystemId:(uint8_t)systemId componentId:(uint8_t)componentId usec:(uint64_t)usec x:(float)x y:(float)y z:(float)z covariance:(float *)covariance {
  if ((self = [super init])) {
    mavlink_msg_vision_speed_estimate_pack(systemId, componentId, &(self->_message), usec, x, y, z, covariance);
  }
  return self;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"%@, usec=%lld, x=%f, y=%f, z=%f, covariance=%@", [super description], [self usec], [self x], [self y], [self z], @"[array of float[9]]"];
}
