...
- (id)initWithSystemId:(uint8_t)systemId componentId:(uint8_t)componentId streamId:(uint8_t)streamId count:(uint8_t)count flags:(VIDEO_STREAM_STATUS_FLAGS)flags framerate:(float)framerate resolutionH:(uint16_t)resolutionH resolutionV:(uint16_t)resolutionV bitrate:(uint32_t)bitrate rotation:(uint16_t)rotation hfov:(uint16_t)hfov uri:(char *)uri {
  if ((self = [super init])) {
    mavlink_msg_video_stream_information_pack(systemId, componentId, &(self->_message), streamId, count, flags, framerate, resolutionH, resolutionV, bitrate, rotation, hfov, uri);
  }
  return self;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"%@, streamId=%d, count=%d, flags=%d, framerate=%f, resolutionH=%d, resolutionV=%d, bitrate=%d, rotation=%d, hfov=%d, uri=%@", [super description], [self streamId], [self count], [self flags], [self framerate], [self resolutionH], [self resolutionV], [self bitrate], [self rotation], [self hfov], [self uri]];
}
