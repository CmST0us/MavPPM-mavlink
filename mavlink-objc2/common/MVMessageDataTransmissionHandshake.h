...
- (id)initWithSystemId:(uint8_t)systemId componentId:(uint8_t)componentId type:(DATA_TYPES)type size:(uint32_t)size width:(uint16_t)width height:(uint16_t)height packets:(uint16_t)packets payload:(uint8_t)payload jpgQuality:(uint8_t)jpgQuality;
