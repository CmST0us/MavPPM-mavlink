...
- (id)initWithSystemId:(uint8_t)systemId componentId:(uint8_t)componentId id:(uint8_t)id batteryFunction:(MAV_BATTERY_FUNCTION)batteryFunction type:(MAV_BATTERY_TYPE)type temperature:(int16_t)temperature voltages:(uint16_t *)voltages currentBattery:(int16_t)currentBattery currentConsumed:(int32_t)currentConsumed energyConsumed:(int32_t)energyConsumed batteryRemaining:(int8_t)batteryRemaining timeRemaining:(int32_t)timeRemaining chargeState:(MAV_BATTERY_CHARGE_STATE)chargeState {
  if ((self = [super init])) {
    mavlink_msg_battery_status_pack(systemId, componentId, &(self->_message), id, batteryFunction, type, temperature, voltages, currentBattery, currentConsumed, energyConsumed, batteryRemaining, timeRemaining, chargeState);
  }
  return self;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"%@, id=%d, batteryFunction=%d, type=%d, temperature=%d, voltages=%@, currentBattery=%d, currentConsumed=%d, energyConsumed=%d, batteryRemaining=%d, timeRemaining=%d, chargeState=%d", [super description], [self id], [self batteryFunction], [self type], [self temperature], @"[array of uint16_t[10]]", [self currentBattery], [self currentConsumed], [self energyConsumed], [self batteryRemaining], [self timeRemaining], [self chargeState]];
}
