...
- (id)initWithSystemId:(uint8_t)systemId componentId:(uint8_t)componentId timeUsec:(uint64_t)timeUsec frameId:(MAV_FRAME)frameId childFrameId:(MAV_FRAME)childFrameId x:(float)x y:(float)y z:(float)z q:(float *)q vx:(float)vx vy:(float)vy vz:(float)vz rollspeed:(float)rollspeed pitchspeed:(float)pitchspeed yawspeed:(float)yawspeed poseCovariance:(float *)poseCovariance twistCovariance:(float *)twistCovariance {
  if ((self = [super init])) {
    mavlink_msg_odometry_pack(systemId, componentId, &(self->_message), timeUsec, frameId, childFrameId, x, y, z, q, vx, vy, vz, rollspeed, pitchspeed, yawspeed, poseCovariance, twistCovariance);
  }
  return self;
}
...
- (uint16_t)twistCovariance:(float *)twist_covariance {
  return mavlink_msg_odometry_get_twist_covariance(&(self->_message), twist_covariance);
}

- (NSString *)description {
  return [NSString stringWithFormat:@"%@, timeUsec=%lld, frameId=%d, childFrameId=%d, x=%f, y=%f, z=%f, q=%@, vx=%f, vy=%f, vz=%f, rollspeed=%f, pitchspeed=%f, yawspeed=%f, poseCovariance=%@, twistCovariance=%@", [super description], [self timeUsec], [self frameId], [self childFrameId], [self x], [self y], [self z], @"[array of float[4]]", [self vx], [self vy], [self vz], [self rollspeed], [self pitchspeed], [self yawspeed], @"[array of float[21]]", @"[array of float[21]]"];
}
