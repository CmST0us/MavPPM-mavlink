...
- (id)initWithSystemId:(uint8_t)systemId componentId:(uint8_t)componentId timeUsec:(uint64_t)timeUsec fixType:(GPS_FIX_TYPE)fixType lat:(int32_t)lat lon:(int32_t)lon alt:(int32_t)alt eph:(uint16_t)eph epv:(uint16_t)epv vel:(uint16_t)vel cog:(uint16_t)cog satellitesVisible:(uint8_t)satellitesVisible altEllipsoid:(int32_t)altEllipsoid hAcc:(uint32_t)hAcc vAcc:(uint32_t)vAcc velAcc:(uint32_t)velAcc hdgAcc:(uint32_t)hdgAcc {
  if ((self = [super init])) {
    mavlink_msg_gps_raw_int_pack(systemId, componentId, &(self->_message), timeUsec, fixType, lat, lon, alt, eph, epv, vel, cog, satellitesVisible, altEllipsoid, hAcc, vAcc, velAcc, hdgAcc);
  }
  return self;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"%@, timeUsec=%lld, fixType=%d, lat=%d, lon=%d, alt=%d, eph=%d, epv=%d, vel=%d, cog=%d, satellitesVisible=%d, altEllipsoid=%d, hAcc=%d, vAcc=%d, velAcc=%d, hdgAcc=%d", [super description], [self timeUsec], [self fixType], [self lat], [self lon], [self alt], [self eph], [self epv], [self vel], [self cog], [self satellitesVisible], [self altEllipsoid], [self hAcc], [self vAcc], [self velAcc], [self hdgAcc]];
}
